...
const AnalysisResultCard: React.FC<AnalysisResultCardProps> = ({ result, loading, error }) => {
  const { t, i18n } = useTranslation();

  // Construct the narrative for TTS
  const speechText = useMemo(() => {
    ...
    const matchLabel = `${t('common.match')}: ${Math.round(result.confidence * 100)} percent`;
    
    let text = `${t('crop.health_report')}. ${t('dashboard.cards.prices')}: ${cropLabel}. ${t('common.results')}: ${statusLabel}. ${matchLabel}. ${result.summary}. `;
  ...
  if (loading) {
    return (
      <div className="h-full flex flex-col items-center justify-center text-center p-12 glass rounded-3xl border border-emerald-500/20 bg-emerald-500/5 animate-pulse">
        ...
        <h3 className="text-xl font-bold text-emerald-400 mb-2">{t('crop.analyzing')}</h3>
        <p className="text-slate-400 text-sm max-w-xs">
          {t('crop.analyzing_hint')}
        </p>
      </div>
    );
  }
  ...
  if (error) {
    return (
      <div className="h-full flex flex-col items-center justify-center text-center p-12 glass rounded-3xl border border-red-500/20 bg-red-500/5">
        ...
        <h3 className="text-xl font-bold text-red-400 mb-2">{t('crop.failed')}</h3>
  ...
  if (!result) {
    return (
      <div className="h-full flex flex-col items-center justify-center text-center p-12 glass rounded-3xl border border-dashed border-slate-800 opacity-60">
        ...
        <h3 className="text-xl font-bold text-slate-400 mb-2">{t('crop.diagnosis_ready')}</h3>
        <p className="text-slate-500 text-sm max-w-xs">
          {t('crop.diagnosis_hint')}
        </p>
      </div>
    );
  }
  ...
  const getSeverityColor = (severity: string) => {
    if (isHealthy) return 'text-emerald-500 bg-emerald-500/10 border-emerald-500/20';
  ...
  return (
    <div className="glass p-8 rounded-3xl border border-slate-800 space-y-6 animate-in slide-in-from-right-4 duration-500 shadow-xl">
      <div className="flex items-center justify-between flex-wrap gap-4">
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="flex items-center gap-2">
             <span className={`px-3 py-1 rounded-full text-[10px] font-bold border uppercase tracking-widest ${getSeverityColor(result.severity)}`}>
      ...
      <div className={`p-6 rounded-2xl border transition-colors ${isHealthy ? 'bg-emerald-500/10 border-emerald-500/20' : 'bg-slate-900/50 border-slate-800'}`}>
        ...
        <div className="w-full bg-slate-800 h-1.5 rounded-full overflow-hidden">
          <div 
            className={`h-full transition-all duration-1000 ${isHealthy ? 'bg-emerald-500' : 'bg-emerald-500'}`} 
      ...
      <div className="space-y-4">
        ...
        <div className={`p-5 rounded-2xl border ${isHealthy ? 'bg-emerald-500/5 border-emerald-500/10' : 'bg-slate-800/30 border-slate-800'}`}>
          <div className="flex items-center justify-between mb-3">
            <h5 className="text-slate-300 font-bold text-xs flex items-center gap-2 uppercase tracking-widest">
              <Droplets className="w-4 h-4 text-blue-400" /> {isHealthy ? t('crop.maintenance') : t('crop.prevention')}
            </h5>
          </div>
